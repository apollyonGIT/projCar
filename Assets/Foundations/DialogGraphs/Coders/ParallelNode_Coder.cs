...
{
    public class ParallelNode_Coder : IDialogNode_Coder
    {
        ...
        string IDialogNode_Coder.key_name => "ParallelNode";
        ...
        {
            var output_ac_list = (List<(string, Func<object>)>)m_fields["output_ac_list"];
            foreach (var (_, output_ac) in output_ac_list)
            {
                output_ac?.Invoke();
            }
