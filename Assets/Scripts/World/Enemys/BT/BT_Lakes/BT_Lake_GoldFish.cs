...
using UnityEngine;
using World.Helpers;
using static World.Enemys.BT.BT_Lake_GoldFish;

namespace World.Enemys.BT
{
    public class BT_Lake_GoldFish : Enemy_BT, IEnemy_BT_FlyAround, IEnemy_BT_FlyFollow
    {
        #region FSM
        public enum EN_FSM
        {
        ...
        {
            cell.position_expt = cell.target.Position + new Vector2(0, 2);

            Road_Info_Helper.try_get_altitude(cell.pos.x, out var road_y);
            cell.pos.y = Mathf.Max(cell.pos.y, road_y + 3f);

            if (Vector2.Distance(cell.pos, cell.target.Position) <= m_flyFollow_dis + 2f)
            {
