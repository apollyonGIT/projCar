...
{
    public class Shooter_AutoGun_Pistol : Shooter_AutoGun
    {

        public Device_Attachment_Tracked_Switching_Silo Dual_Track_Clip = new();
        ...
        protected override void tick_after_fsm_while_unbroken()
        {
            ...
            if (Dual_Track_Clip.Silo_Fully_Open)
            {
            ...
            if (!Dual_Track_Clip.Silo_Fully_Installed)
        ...
        {
            return base.can_successfully_load_bullet_into_barrel() && Dual_Track_Clip.Silo_Fully_Installed;
        }
        ...
        {
            if (current_ammo == 0 && !Dual_Track_Clip.Silo_Fully_Open)
            {
                Dual_Track_Clip.Fully_Open();
            ...
            {
                if (!Dual_Track_Clip.Silo_Fully_Installed)
                    Dual_Track_Clip.Fully_Install();// 工作：尝试关闭弹匣
                else
        ...
        protected override bool Auto_Attack_Job_Content()
        {
            switch (barrel_bullet_stage)
            {
                case Barrel_Ammo_Stage.empty:
                    if (Dual_Track_Clip.Silo_Fully_Installed && current_ammo > 0)
